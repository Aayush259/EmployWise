...
export default function UserList() {
    ...
    const { userList, fetchingUserList, currentPage, totalPages, error, setCurrentUser, nextPage, prevPage } = useUser();
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8 px-4 md:px-8 lg:px-16">
                            {
                                userList.map((user) => (
                                    <div key={user.id} className="bg-gray-800 rounded-lg p-4 flex flex-col items-center group relative shadow-in duration-300">
                                        <button className="opacity-0 group-hover:opacity-100 duration-200 absolute top-4 right-4 cursor-pointer hover:opacity-60" onClick={() => setCurrentUser(user.id)}>
                                            <GoPencil size={20} />
                                        </button>
                                        <img
                                            src={user.avatar}
